...
class Wblogin():
    ...
    def getScan(self):
        ...
        print(self.loginDic)
        info = self.loginDic['reason']
        token = info.split("%D")[-1][:-1]  # 提取token
        print("token: ", token)
    ...
    def verify(self, token, veUrl):
        ...
        headers = {
            ...
            'Referer': 'https://passport.weibo.com/protection/index?token={}&callback_url=https%3A%2F%2Fweibo.com'.format(
                token),
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.114 Safari/537.36 Edg/91.0.864.59',
    ...
    def crossDomain1(self, redirectionUrl):
        ...
        # print(alt)  # 处理alt
        # 拿到通行证
        url = "https://login.sina.com.cn/sso/login.php?entry=weibo&returntype=META&crossdomain=1&cdult=3&savestate=30&alt={}&url=https://weibo.com".format(
            alt.replace("ST", "ALT"))
